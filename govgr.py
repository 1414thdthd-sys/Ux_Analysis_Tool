...
def safe_median(series):
    s = pd.to_numeric(series, errors="coerce")
    return s.median() if not s.dropna().empty else np.nan


def safe_std(series):
    s = pd.to_numeric(series, errors="coerce")
    return s.std() if not s.dropna().empty else np.nan
...
def main():
    ...
    for pdir in participants:
        ...
        if not segments:
            log(f"[WARN] No G-task segments found for {participant}")
            continue
        ...
        for seg in segments:
            ...
            row["HR_Delta"] = row["HR_Mean"] - row["HR_Baseline"] if pd.notna(row["HR_Mean"]) and pd.notna(row["HR_Baseline"]) else np.nan
            row["IBI_Delta"] = row["IBI_Mean"] - row["IBI_Baseline"] if pd.notna(row["IBI_Mean"]) and pd.notna(row["IBI_Baseline"]) else np.nan
    ...
    master_path = OUTPUT_DIR / "govgr_master_dataset.csv"
    summary_path = OUTPUT_DIR / "govgr_task_summary.csv"
    ...
    if not master_df.empty:
        ...
        summary_bar(summary, "Task", "CompletionTimeMean_sec", "Govgr Mean Completion Time", "summary_completion_time.png")
        summary_bar(summary, "Task", "SuccessRate", "Govgr Success Rate", "summary_success_rate.png")
        summary_bar(summary, "Task", "CogLoadMean", "Govgr Mean Cognitive Load", "summary_cognitive_load.png")
        summary_bar(summary, "Task", "CogLoadPeakMean", "Govgr Mean Cognitive Peak", "summary_cognitive_peak.png")
        summary_bar(summary, "Task", "HR_DeltaMean", "Govgr Mean HR Delta", "summary_hr_delta.png")
        summary_bar(summary, "Task", "HR_MeanMean", "Govgr Mean HR", "summary_hr_mean.png")
        summary_bar(summary, "Task", "EDA_TonicMean", "Govgr Mean EDA Tonic", "summary_eda_tonic.png")
        summary_bar(summary, "Task", "EDA_PeakRateMean", "Govgr Mean EDA Peak Rate", "summary_eda_peakrate.png")
        summary_bar(summary, "Task", "ConfusedMean", "Govgr Mean Confusion", "summary_confusion.png")
        summary_bar(summary, "Task", "EstimatedFixationEpisodesMean", "Govgr Estimated Fixation Episodes", "summary_fixation_episodes.png")
        summary_bar(summary, "Task", "EstimatedFixationMeanDuration_ms_Mean", "Govgr Mean Fixation Duration", "summary_fixation_duration.png")
    else:
